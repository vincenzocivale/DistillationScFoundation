...
def predict_embeddings(cfg: DictConfig) -> None:
    ...
    if gene_id_key == "_index":
        gene_source = adata.var.index
    else:
        # Se non è _index, cerca la colonna specifica. Se fallisce qui, la chiave è errata.
        gene_source = adata.var[gene_id_key]

    adata.var["id_in_vocab"] = [
        vocab[gene] if gene in vocab else -1 for gene in gene_source
    ]
    ...
    if gene_id_key == "_index":
        genes = adata.var.index.tolist() # Ricalcola l'indice dopo il filtro sopra
    else:
