...
# Removed llmfoundry.layers_registry import param_init_fns
...
from tahoe_x1.model.blocks import (
    ...
    _get_activation_fn, # Import helper from blocks
    _get_norm_layer, # Import helper from blocks
    # Removed _init_weights import
)
from tahoe_x1.tokenizer import GeneVocab

log = logging.getLogger(__name__)

# --- Helper functions to replace llmfoundry utilities (copied locally for TXModel) ---

def _init_weights(module: nn.Module, n_layers: int, d_model: int, init_config: Dict[str, Any]):
    """Basic weight initialization function (copied from StudentTXModel for TXModel)."""
...
class TXModel(nn.Module):
    def __init__(
        ...
        # Removed use_chem_token and its assertions
        ...
        if self.init_device != "meta":
            ...
            self.apply(lambda module: _init_weights(module, self.n_layers, self.d_model, self.init_config))
...
class ComposerTX(ComposerModel):
    ...
    def from_hf(
        ...
        model.load_state_dict(model_state_dict, strict=True) # Changed strict to True as we expect clean model
