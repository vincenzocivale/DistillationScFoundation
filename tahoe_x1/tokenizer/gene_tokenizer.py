...
class GeneVocab:
    ...
    def to_ids(self, tokens: Iterable[str]) -> List[int]:
        ...
        Unknown tokens are assigned the default index (if set).
        """
        return [self[token] for token in tokens]

    def to_seq_ids(self, tokens: Iterable[str]) -> List[int]:
        """Alias kept for compatibility with existing codebases."""
